...
function uniqueValues(array){
    let dataset = new Set();
    array.forEach((item) => dataset.add(item));
    return dataset.size;
}
...
//MUTATE - DELETE/REMOVE ELEMENT TO AVOID DUPLICITY ON MULTIPLE ITERATIONS !!!!!!
function countPairs(array, number){
    let count = 0;
    let dataset = new Set(array);
    //provide set items directly as an argument
    //array.forEach((item) => dataset.add(item));
    for (let value of dataset) {
        //remove element first
        dataset.delete(value);
        if (dataset.has(number - value)) count++;
    }
    return count;
}
